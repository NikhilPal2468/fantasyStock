...
if ($isEmailConfirmed == 1) {
    ...
    $Dates = array();
    for ($i = 1; $i <= $a; $i++) {
        $query = "SELECT * FROM live_game where Game_Id  =$i AND Version='nasdaq'";
        $run = mysqli_query($connect, $query);
        $row = mysqli_fetch_array($run);
...
?>
    ...
    <body>
        ...
        <div class="main-div" onclick="removenotificationtable()">
            ...
            <div class="game-type-selection">
                ...
                <a href="nasdaq.php"><button class="btnSelection" id="btnActive">Nasdaq</button></a>
    ...
    <script>
        ...
        for (var i = 1; i <= total; i++) {
            ...
            link.href = "Nasdaq_List.php?gid=<?php echo $gameId[0]; ?>";
            ...
            document.getElementById(linkid).setAttribute('href', 'Nasdaq_List.php?gid=' + gameId[i - 1]);
                ...
                for (var k = 0; k < playersId[j].length - 1; k++) {
                    for (var l = 0; l < gameIds.length; l++) {
                        if (playersId[j][k] == uid && gameIds[l] === gameId[i - 1]) {
                            ...
                            document.getElementById(linkid).setAttribute('href', 'Edit_Nasdaq_List.php?gid=' + gameId[i - 1]);
                        }
                    }
                }
            }
